...
import { GraduationCap, Plus, Pencil, Clock, Users } from 'lucide-react';
...
const PLACEHOLDER_COURSES: CourseRow[] = [
  { id: 'demo-1', module_number: 1, title: 'Drone Inspection Fundamentals', description: 'FAA Part 107 essentials, capture patterns, photo protocols, GPS metadata, and platform integration.', duration_min: 95, enrolled: 48, status: 'published' },
  { id: 'demo-2', module_number: 2, title: 'Insurance Claim Process', description: 'Filing storm/wind/hail claims, working with adjusters, supplementing for code upgrades, ACV vs RCV.', duration_min: 110, enrolled: 36, status: 'published' },
  { id: 'demo-3', module_number: 3, title: 'Storm Damage Assessment', description: 'Identify hail bruising, wind uplift, granular loss, and impact patterns for credible documentation.', duration_min: 80, enrolled: 41, status: 'draft' },
];
    ...
    <main className="p-8 max-w-6xl mx-auto">
      <header className="mb-8 flex items-end justify-between">
        <div>
          ...
          <p className="text-[#E5E9F2]/60 text-sm">{courses.length} modules · {totalEnrolled.toLocaleString()} enrollments</p>
        </div>
          ...
          <tbody className="divide-y divide-[#E5E9F2]/5">
            {courses.map(c => (
              <tr key={c.id} className="hover:bg-white/5 transition">
                ...
                <td className="px-6 py-4 text-sm text-[#E5E9F2]/70"><div className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" />{c.duration_min || '—'}m</div></td>
                <td className="px-6 py-4 text-sm text-[#E5E9F2]/70"><div className="flex items-center gap-1.5"><Users className="w-3.5 h-3.5" />{c.enrolled}</div></td>
                <td className="px-6 py-4">
